...
// test name
#define CPPWEB_TEST_NAME_CREATE(test_group, test_name)                         \
...
#define INIT(init_name)                                                        \
  ...
  auto CPPWEB_TEST_INIT_NAME_CREATE(init_name) =                               \
      CPPWEB_TEST_INIT_NAME(init_name)();                                      \
  int CPPWEB_TEST_INIT_NAME(init_name)()
...
#define TEST(test_group, test_name)                                            \
  ...
  CPPWEB_TEST_NAME(test_group, test_name)                                      \
  CPPWEB_TEST_NAME_CREATE(test_group, test_name);                              \
  void CPPWEB_TEST_NAME(test_group, test_name)::TestBody()

// some function for debug and judge
#define SKIP()                                                                 \
  TESTSTDOUT(TESTYELLOW("[SKIP] in [" << _FILE_LINE_MSG_ << "] : "));          \
  return;
#define DEBUG(text)                                                            \
  TESTSTDOUT(TESTYELLOW("[DEBUG] in [" << _FILE_LINE_MSG_ << "] : " << text))
#define ERROR(text)                                                            \
  TESTSTDERR(TESTCAR("[ERROR] in [" << _FILE_LINE_MSG_ << "] : " << text))     \
  _CLASS_FAIL_
#define FATAL(text)                                                            \
  TESTSTDERR(TESTRED("[FATAL] in [" << _FILE_LINE_MSG_ << "] : " << text))     \
  _CLASS_FAIL_;                                                                \
  return;
#define PANIC(text)                                                            \
  TESTSTDERR(TESTRED("[PANIC] in [" << _FILE_LINE_MSG_ << "] : " << text));    \
  exit(-1);
#define EXPECT_EQ(result, expect)                                              \
  if (result != expect) {                                                      \
    FATAL(CPPWEBCONNECTSTR(result want get expect but get) << " " << result)   \
  }
#define MUST_EQUAL(result, expect)                                             \
  if (result != expect) {                                                      \
    FATAL(result << " " << CPPWEBCONNECTSTR(!= expect));                       \
  }
